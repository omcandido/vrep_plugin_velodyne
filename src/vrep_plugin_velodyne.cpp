...
#include "../include/vrep_plugin_velodyne/vrep_plugin_velodyne.h"
...
VREP_DLLEXPORT unsigned char v_repStart(void* reservedPointer,int reservedInt)
{
	...
	if (vrepLib==NULL)
	{
        std::cout << "Error, could not find or correctly load the V-REP library. Cannot start 'rosVelodyne' plugin.\n";
		return(0); // Means error, V-REP will unload this plugin
	}
	if (getVrepProcAddresses(vrepLib)==0)
	{
        std::cout << "Error, could not find all required functions in the V-REP library. Cannot start 'rosVelodyne' plugin.\n";
	...
	{
        std::cout << "Sorry, your V-REP copy is somewhat old. Cannot start 'rosVelodyne' plugin.\n";
		unloadVrepLibrary(vrepLib);
		return(0); // Means error, V-REP will unload this plugin
	}
	// ******************************************
